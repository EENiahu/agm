        ...
        <v-row class="mb-6">
          ...
          <v-col cols="6" align="end">
            <template v-if="hasQuestions">
              <router-link :to="{name: 'pollsEdit', params: {meetingId: MeetingId}}" class="mr-4">
                <v-btn
                    type="button"
                    class="px-10"
                    color="blue-grey darken-4 white--text"
                    depressed
                    rounded>Update Polls</v-btn>
              </router-link>
            </template>
...
<script>
  ...
  export default {
    ...
    computed: {
      hasQuestions() {
        return this.meeting.questions ? this.meeting.questions.length > 0 : false;
      },
    ...
    created() {
      Promise.all([this.getStates(), this.getTimezones(), this.getProperties(), this.getMeeting()])
          .then(res => {
    ...
    methods: {
      ...
      getMeeting() {
        return apiMeetings.getOne(this.MeetingId, 'questions')
          .then(res => {
