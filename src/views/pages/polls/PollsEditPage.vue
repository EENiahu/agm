...
export default {
  ...
  methods: {
    ...
    addQuestion(scroll = true) {
      this.$store.dispatch('questions/add_question', this.meetingId);
    ...
    getQuestions() {
      apiQuestions.getAll()
          .then(res => {
            ...
            const questions = res.data.length ? res.data : [{}];
            this.$store.dispatch('questions/set_questions', questions);
          })
          .catch(err => {
            console.error(err);
          })
